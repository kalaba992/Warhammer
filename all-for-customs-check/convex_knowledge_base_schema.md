# Convex Knowledge Base – Audit‑Grade Schema & Models

Ovaj dokument sadrži **tačnu, implementabilnu specifikaciju** Convex baze znanja (System of Record) za Carinski‑Assistent.
...
- multi‑tenant
- spreman za 0/24 rad
- bez halucinacija i bez AI odluka

---

## 1. Princip dizajna (kratko i jasno)

- **Convex = System of Record (SoT)**
- Svi dokumenti, chunkovi, citati i odluke su **nepobitna istina**
- Search servisi (BM25 / vector) su samo derivati, nikad autoritet
- Svaka FINAL odluka mora imati:
  - citation_id
  - snapshot_pointer
  - corpus_version
  - hash
...
## 2. Convex schema (tables + indexes)
...
## 3. Deterministička ID konvencija (OBAVEZNO)

```text
document_id  = doc_<sha256(source_url)[:12]>
chunk_id     = chk_<sha256(document_id:ordinal:text_hash)[:16]>
citation_id  = cit_<sha256(chunk_id:snapshot_hash:locator)[:16]>
bundle_id    = eb_<request_id>
```

Isti ulaz → isti ID. Nema duplikata. Nema haosa.

---

## 4. Šta ovaj model garantuje

- audit trail (hash + snapshot + corpus_version)
- STOP kad nema dokaza
- 100% reproducibilnost
- legal‑safe arhitekturu
- spremno za hybrid search (BM25 + vector)

---

## 5. Status

Ovo je **finalna SoT specifikacija**. Možeš je direktno ubaciti u repo i početi implementaciju ingestion i indexing workera.

Sljedeći sloj (ako zatreba):
- ingestion worker (internet → Convex)
- indexing worker (Convex → search indeksi)

Ali baza je ovim **zaključana i ispravna**.
