...
type TooltipProps = {
  active?: boolean
  payload?: RechartsPayload<number, string>[]
  label?: string | number
  labelFormatter?: (label: TooltipProps['label'], payload: RechartsPayload<number, string>[]) => ReactNode
  formatter?: (
    value: RechartsPayload<number, string>['value'],
    name: RechartsPayload<number, string>['name'],
    item: RechartsPayload<number, string>,
    index: number,
    payload: RechartsPayload<number, string>[]
  ) => ReactNode
  className?: string
...
function ChartTooltipContent(props: TooltipProps) {
  ...
  const safePayload = useMemo<RechartsPayload<number, string>[]>(
    () => payload ?? [],
    [payload]
  )
  ...
  const tooltipLabel = useMemo(() => {
    ...
    const value =
      !labelKey && typeof label === "string"
        ...
        : itemConfig?.label
    const safeLabelValue =
      typeof value === "string" || typeof value === "number" ? value : label

    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {labelFormatter(safeLabelValue, safePayload)}
        </div>
      )
    }
